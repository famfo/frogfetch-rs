...
pub fn get_info() {
    ...
    // Get the os using sw_vers -productName
    // and the version using sw_vers -productVersion
    ...
    let os_version = String::from_utf8(
        Command::new("sw_vers")
            .arg("-productVersion")
            .output()
            .expect("Failed to execute sw_vers")
    ...
    // Get the kernel name using kern.ostype
    // and the kernel version using sysctl -n kern.ostype
    ...
    let kernel = format!("{} {}", kernel, kernel_version);
